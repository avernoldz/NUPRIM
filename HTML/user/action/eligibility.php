...
if (isset($_POST['add-eligibility'])) {
    ...
    $insert = "INSERT INTO eligibility (userid, rating, eligibility, licenseNo, dateOfExam, placeOfExam, validity) 
        VALUES ('$userid', '$sanitized_rating', '$sanitized_eligibility', '$sanitized_license', '$sanitized_date', '$sanitized_place' , '$sanitized_vdate')";

    if (mysqli_query($conn, $insert)) {
        logAction($conn, $_SESSION['userid'], 'Add new eligibility', $_SESSION['type']);
        echo "<script>window.location.href='../eligibility.php?userid=$userid&alert=1';</script>";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if (mysqli_query($conn, $delete)) {
        ...
        header("Location:../eligibility.php?userid=$userid&alert=1");
    } else {
